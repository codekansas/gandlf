...
class RecurrentAttention1D(keras.layers.Wrapper):
    ...
    a vector `s`, which is given by:
        ...
        m = attn_activation(dot(h, U_m) * dot(attention, U_a) + b_a)
    ...
    def build(self, input_shape):
        ...
        self.trainable_weights = [self.U_m, self.U_a, self.b_a,
                                  self.U_s, self.b_s]
    ...
    def _compute_attention(self, h, attention):
        m = self.attn_activation(K.dot(h, self.U_m) * attention + self.b_a)
